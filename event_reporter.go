...
func ReportDataEvent(event *pktlatencyKernEvt, conn *Connection4) error {
	...
	var direct uint32
	if conn.role == pktlatencyEndpointRoleTKRoleClient {
		if uint32(event.ConnIdS.Direct) == uint32(pktlatencyTrafficDirectionTKIngress) {
			direct = uint32(toSrc)
		} else {
			direct = uint32(toDst)
		}
...
func ReportConnEvent(event *pktlatencyConnEvtT) error {
