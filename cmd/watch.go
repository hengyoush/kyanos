...
func init() {
	...
	watchCmd.PersistentFlags().BoolVar(&options.TraceDevEvent, "trace-dev-event", true, "Collect dev layer events to measure network interface time spent.")
	watchCmd.PersistentFlags().BoolVar(&options.TraceSocketEvent, "trace-socket-event", true, "Collect socket layer events to measure the time spent on socket data copying.")
	watchCmd.PersistentFlags().BoolVar(&options.TraceSslEvent, "trace-ssl-event", true, "Collect SSL events to trace SSL connection data.")
